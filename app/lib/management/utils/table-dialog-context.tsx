...
const TableDialogContext =
  createContext<TableDialogContextType | null>(null);

export function TableDialogProvider({
  children,
}: {
  children: ReactNode;
}) {
...
export function useTableDialog() {
  ...
  if (!context) {
    throw new Error(
      'useTableDialog must be used within TableDialogProvider'
    );
  }
  return context;
}
